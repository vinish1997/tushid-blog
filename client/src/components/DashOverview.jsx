...
export default function DashOverview() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch("/api/v1/users?limit=5");
        const data = await res.json();
        if (res.status === 200) {
          setUsers(data.users);
          setTotalUsers(data.totalCount);
          setLastMonthUsers(data.lastMonthCreatedUsers);
        }
      } catch (error) {
        console.log(error);
      }
    ...
    const fetchPosts = async () => {
      try {
        const res = await fetch("/api/v1/posts?limit=5");
        const data = await res.json();
        if (res.status === 200) {
          setPosts(data.posts);
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch("/api/v1/comments?limit=5");
        const data = await res.json();
        if (res.status === 200) {
